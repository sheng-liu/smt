...
##' @usage compareFolder(folders=c(folder1,folder2,...),ab.track=F)
...
compareFolder=function(folders=c(folder1,folder2,...),ab.track=F){
    ...
    if (ab.track==T){
        ...
        for (i in 1:length(folder.list)) {
            sample.list[i]=readDiatrack(
                folder=folder.list[[i]],merg=T,ab.track=T)
        ...
        for (i in 1:length(folder.list)) {
            # i=1
            sample.list[i]=readDiatrack(
                folder=folder.list[[i]],merg=T,ab.track=F)
            cat("\n...\n") # seperator makes ouput clearer
            names(sample.list)[i]=names(folder.list)[i]
        }
    }
